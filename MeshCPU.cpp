...
void MeshCPU::ReadMesh(const char *filename, const char *package, const char *tag)
{
	...
	PositionBufferCPU* pVB = m_hPositionBufferCPU.getObject<PositionBufferCPU>();
	float xMin = 10000;
	float xMax = -10000;
	float yMin = 10000;
	float yMax = -10000;
	...
	for (int i = 0; i < pVB->m_values.m_size / 3; i++) {
		/*Vector3 pos(pVB->m_values[i * 3], pVB->m_values[i * 3 + 1], pVB->m_values[i * 3 + 2]);*/
	...
	m_AABBCoords[0] = Vector3(xMin, yMin, zMin);
	m_AABBCoords[1] = Vector3(xMax, yMin, zMin);
	m_AABBCoords[2] = Vector3(xMax, yMin, zMax);
	m_AABBCoords[3] = Vector3(xMin, yMin, zMax);
	m_AABBCoords[4] = Vector3(xMin, yMax, zMax);
	m_AABBCoords[5] = Vector3(xMax, yMax, zMax);
	m_AABBCoords[6] = Vector3(xMax, yMax, zMin);
	m_AABBCoords[7] = Vector3(xMin, yMax, zMin);
